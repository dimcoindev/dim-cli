...
class Command extends BaseCommand {
    ...
    constructor(npmPack) {
        ...
        this.signature = "crawler";
        this.description = ("    " + "This tool lets you export data of any mosaic on the NEM blockchain.\n"
                    + "    " + "Examples:\n\n"
                    + "    " + "  $ dim-cli crawler --format xlsx --currency dim:coin");

        this.options = [{
            "signature": "-h, --help",
            "description": "Print help message about the `dim-cli.js crawler` command."
        }, {
            ...
            "signature": "-c, --currency <mosaic>",
            "description": "Set the Fully Qualified Mosaic Name for which to crawl transactions."
        }, {
            "signature": "-A, --minAmount <amount>",
            "description": "Set a minimum amount condition (dim:token '50' for example)."
        }];

        this.examples = [
            "dim-cli crawler",
            "dim-cli crawler --format xlsx",
        ];
    ...
    /**
     * This method will run the DIM Mosaic Crawler. 
     * 
     * This can  be used for any NEM Blockchain Mosaic.
     *
     * @param   {object}    env
     * @return  void
     */
    async run(env) {
        ...
        this.log("");
        this.log("DIM Crawler (Crawler for NEM Blockchain Mosaic) v" + this.npmPackage.version);
        this.log("");
        ...
        let name = env.file || "dim-mosaic-crawler-accounts-" + utc;
        let trx_name = env.trxlog || "dim-mosaic-crawler-transactions-" + utc;
        let currency = env.currency || "dim:coin";
        let minAmount = env.minAmount || 0;
        ...
        if (! /([a-z0-9A-Z\-_\.]):([a-z0-9A-Z\-_\.])/.test(currency)) {
            console.log("Invalid NEM Blockchain Mosaic Fully Qualified Name provided in --currency.");
            return this.end();
        }

        this.currency = currency;

        // get necessary Currency Parameters
        let mosaicParams = await this.explorer.getCurrency(currency);

        if (! mosaicParams) {
            console.error("Invalid Fully Qualified Mosaic Name provided in --currency. Could not be found on the NEM Blockchain.");
            return this.end();
        }

        // XXX enable testnet compatibility
        let mosaicCreator = mosaicParams.creator;
        let startAddress = this.api.SDK.model.address.toAddress(mosaicCreator, this.parameters.dimNetworkId);
        ...
        // ----
        this.log("Now starting " + currency + " crawler with address: " + startAddress);
        // ----
        // ----
        // ----
        ...
        try {
            ...
            // STEP 1: fetch first batch of mosaic holders (starting from mosaic creator)
            let totalMosaicHolders = await this.startCrawler(startAddress);

            // DONE CRAWLING blockchain transactions
            this.formatterTrxes.save();

            // this is the total number of people *who once 
            // the said currency* but maybe don't have them anymore.
            let unfilteredMosaicHoldersCount = Object.keys(this.storage.accounts).length;

            // STEP 2: FILTER all holders (token holders must have more than 50 dim:token)
            let elligibleMosaicHoldersCount = await this.filterByMosaicHolderElligibility(Object.keys(this.storage.accounts), this.storage.mosaicHolders);
            ...
            console.log("");
            console.log("Total " + currency + + " Network Depth: " + this.current);
            console.log("All-Time Mosaic Holders: " + unfilteredMosaicHoldersCount + " accounts");
            console.log("Current count of Mosaic Holders: " + elligibleMosaicHoldersCount + " mosaic holders");
            console.log("");
    ...
    async startCrawler(forAddress, lvl) {
        ...
        let MosaicHolders = await this.crawlAddress(forAddress, null);
        let levelHolders = Object.keys(MosaicHolders).slice(beforeThisLevel);

        // update storage
        totalMosaicHolders += levelHolders.length;
     ...
     * @return {Integer}
     */
    async filterByMosaicHolderElligibility(addresses, holders) {
        let remaining = addresses || [];
        let dimMosaicHolders = holders;
        ...
        for (let b = 0; b < balances.length; b++) {
            ...
            if (s !== this.currency) continue;

            if (balances[b].quantity < this.parameters.minMosaicHolderShare) {
                // token holder DOES NOT meet requirement.
                continue;
            }

            let formattedBalance = (balances[b].quantity / Math.pow(10, 6)).toFixed(6);
            console.log("Found DIM.MosaicHolder with " + address 
                        + " and " + formattedBalance + " " + this.currency);

            // token holder MEETS requirements
            dimMosaicHolders[address] = balances[b].quantity;
        ...
        if (remaining.length) {
            return await this.filterByMosaicHolderElligibility(remaining, dimMosaicHolders);
        }

        // RECURSION DONE
        return Object.keys(dimMosaicHolders).length;
    }
     ...
     * @return {Boolean}
     */
    async saveMosaicHolderCache(object) {

        let holder = new DIM.MosaicHolder({});
        ...
        holder.setAttribute("mosaicAmount", object.stakeQuantity);
    ...
    async generateStatistics() {
        ...
        console.log("Total " + this.currency + ": " + (totalTokens / Math.pow(10, 6)).toFixed(6));
        ...
        for (let j = 0; j < rows.length; j++) {
            ...
            let cached = await this.saveMosaicHolderCache(rows[j]);
        }
